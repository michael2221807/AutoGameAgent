...
- **Check BOTH rosters above before any push**: a character already on either list (even if currently absent) is NOT a new NPC — never push them again; bring them back with `set 社交.关系[名称=X].是否在场 / 位置` instead.
- Only a character absent from both rosters is a new NPC; when a new NPC first appears in the narrative, include `"是否在场": true` and the `"位置"` field directly in the push command.
